...
**Command**:
```bash
python3 kernel_builder.py compile --kernel-name <kernel-name> --arch <target-architecture> [--toolchain-name <toolchain-name>] [--rpi-model <rpi-model>] [--config <config-file-path>] [--generate-ctags] [--build-target <build-targets>] [--threads <number-of-threads>] [--clean]
```

**Example**:
```bash
python3 kernel_builder.py compile --kernel-name jetson --arch arm64 --toolchain-name aarch64-buildroot-linux-gnu --config tegra_defconfig --generate-ctags --build-target kernel,dtbs,modules,bindeb-pkg --threads 4 --clean
```
...
**Command**:
```bash
python3 kernel_deployer.py deploy-device --output-dir <output-directory> --ip <device-ip> --user <user>
```

**Example**:
```bash
python3 kernel_deployer.py deploy-device --output-dir $(pwd)/output/jetson --ip 192.168.1.10 --user ubuntu
```

This command copies the compiled kernel and modules to the specified device over SSH and SCP. The `--user` argument should be set to `ubuntu` for NVIDIA Jetson or `pi` for Raspberry Pi.
