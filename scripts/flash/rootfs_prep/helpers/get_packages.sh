...
# Wipe any previously-fetched packages before re-downloading. Without this,
# a tag that drops a deb (or renames one) leaves the old artefact behind in
# packages/, which then propagates into the rootfs via subsequent cp -r /
# rsync, producing silent stale-deb installs. Better to start from empty.
echo "Wiping local packages/ to avoid stale artefacts from previous tags"
rm -rf packages
...
# Drop the now-redundant zip archives so a subsequent cp -r packages/ ...
# only carries actual debs/whls into the rootfs.
echo "Removing intermediate .zip archives"
find packages -mindepth 2 -maxdepth 2 -name '*.zip' -print -delete
