...
def trigger_can(device):
    ...
    run_command(f"cansend {device} 501##00.00.00.00.00.00.00.00")
    print("Trigger frame sent.")

def monitor_can(device):
    """Monitors CAN bus for errors and traffic."""
    print(f"Monitoring CAN device: {device}")
    print("--- CAN Bus Statistics and Errors ---")
    run_command(f"ip -details -statistics link show {device}")
    print("\n--- Live CAN Traffic (press Ctrl+C to stop) ---")
    try:
...
if __name__ == "__main__":
    ...
    group.add_argument("--monitor", action="store_true", help="Monitor CAN traffic and errors.")
