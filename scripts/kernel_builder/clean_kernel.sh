...
# Script to clean a kernel build directory using the clean target.
# Usage: ./clean_kernel.sh [--help] --kernel-name <name> [OPTIONS]
#
# Arguments:
...
#   --dry-run              Optional argument to simulate the clean cleaning process without making changes.
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        --help)
            echo "Usage: ./clean_kernel.sh --kernel-name <name> [OPTIONS]"
            echo
            echo "Arguments:"
    ...
    echo "  --toolchain-version    Version of the toolchain to use (default: 9.3)."
            echo "  --dry-run              Optional argument to simulate the clean cleaning process without making changes."
            echo
            echo "Description:"
            echo "  This script invokes kernel_builder.py to clean a kernel build directory using the clean target."
            echo
            echo "Examples:"
            echo "  ./clean_kernel.sh --kernel-name jetson"
            echo "  ./clean_kernel.sh --kernel-name my_custom_kernel --arch arm64 --toolchain-name aarch64-toolchain --dry-run"
            exit 0
            ;;
        --kernel-name)
            if [ -n "$2" ]; then
...
COMMAND="python3 \"$KERNEL_BUILDER_PATH\" compile --kernel-name $KERNEL_NAME --arch $ARCH --toolchain-name $TOOLCHAIN_NAME --toolchain-version $TOOLCHAIN_VERSION --build-target clean"
