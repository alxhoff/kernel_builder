...
DEPLOY_SCRIPT="$SCRIPT_DIR/deploy/deploy_kernel.sh"
...
if [ -f "$SCRIPT_DIR/config/device_ip" ]; then
  DEVICE_IP=$(<"$SCRIPT_DIR/config/device_ip")
fi

if [ -f "$SCRIPT_DIR/config/device_username" ]; then
  USERNAME=$(<"$SCRIPT_DIR/config/device_username")
fi
