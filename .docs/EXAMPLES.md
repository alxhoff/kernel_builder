...
Workflows for using `python/kernel_builder.py`, `python/kernel_deployer.py`, and
`python/kernel_debugger.py` on x86 host machines, NVIDIA Jetson boards, and
Raspberry Pi.
...
1. **Build the Docker image**:
   ```bash
   python3 python/kernel_builder.py build
   ```
...
4. **Deploy to the x86 host**:
   ```bash
   python3 python/kernel_deployer.py deploy-x86
   ```
...
4. **Clone overlays / device tree (optional)**:
   ```bash
   python3 python/kernel_builder.py clone-overlays \
     --overlays-url https://github.com/alxhoff/jetson-kernel-overlays \
     --kernel-name jetson

   python3 python/kernel_builder.py clone-device-tree \
...
`python/kernel_debugger.py` wraps common on-device debug chores over SSH:

```bash
python3 python/kernel_debugger.py enable-persistent-logging \
  --ip 192.168.1.10 --user root

python3 python/kernel_debugger.py retrieve-boot-logs \
  --ip 192.168.1.10 --user root \
  --destination-path /tmp/boot_logs

python3 python/kernel_debugger.py record-trace \
  --ip 192.168.1.10 --user root \
...
```bash
python3 python/kernel_builder.py inspect
```

The `storage/kernels/` and `storage/toolchains/` directories are mounted under `/builder`
inside the container.

## Cleaning up Docker resources

```bash
python3 python/kernel_builder.py cleanup
```
