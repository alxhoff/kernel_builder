...
2. **Clone the Kernel Source**:
   ```bash
   python3 kernel_builder.py clone-kernel --kernel-source-url https://github.com/alxhoff/jetson-kernel --kernel-name jetson --git-tag v5.10
   ```

3. **Clone the Toolchain**:
   ```bash
   python3 kernel_builder.py clone-toolchain --toolchain-url https://github.com/alxhoff/Jetson-Linux-Toolchain --toolchain-name aarch64-buildroot-linux-gnu --git-tag v5.10
   ```
...
5. **Clone Device Tree Hardware Repository (Optional)**:
   ```bash
   python3 kernel_builder.py clone-device-tree --device-tree-url https://github.com/alxhoff/jetson-device-tree-hardware --kernel-name jetson
   ```
...
7. **Deploy to NVIDIA Jetson Board**:
   ```bash
   python3 kernel_deployer.py deploy-device --kernel-name jetson --ip 192.168.1.10 --user ubuntu
   ```
...
4. **Deploy to Raspberry Pi**:
   ```bash
   python3 kernel_deployer.py deploy-device --kernel-name rpi --ip 192.168.1.15 --user pi
   ```
