...
def compile_kernel_docker(kernel_name, arch, toolchain_name=None, rpi_model=None, config=None, generate_ctags=False, build_target=None, threads=None, clean=True):
    ...
    docker_command = [
        "docker", "run", "-u", f"{user_id}:{group_id}"
    ] + volume_args + [
        "-w", "/builder", "kernel_builder", "/bin/bash", "-c"
    ]

    base_command = f"make -C /builder/kernels/{kernel_name}/kernel ARCH={arch} -j{threads if threads else '$(nproc)'}"
    ...
    if config:
        combined_command += f"{base_command} {config} && "
    combined_command += f"{base_command} && "  # First make call without a target

    if build_target:
        targets = build_target.split(',')
        for target in targets:
            combined_command += base_command + f" {target} && "
    else:
...
def main():
    ...
    compile_parser.add_argument("--build-target", help="Comma-separated list of build targets (e.g., kernel,dtbs,modules,bindeb-pkg)")
